...
async def get_company_news_async(company_name, api_key='8d71b452dc7640b0b67a3e4d2183a683', num_articles=25):
    # Define the base URL and parameters for the NewsAPI request
    url = "https://newsapi.org/v2/everything"
    params = {
        'q': company_name,  # Search query (company name)
...
# Use asyncio.run() in a sync function to avoid event loop errors
# def gpt_output(company_name):
#     client = genai.Client(api_key="AIzaSyCInk9NK8rrBBmxm3Uk_QHjFygk7fXZrmg")

#     # Run async functions within asyncio.run()
#     prompt = asyncio.run(create_prompt_async(company_name))
